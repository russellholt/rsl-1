// $Id: rsl_controlX.cc,v 1.1 1996/03/18 13:27:28 holtrf Exp holtrf $
//
// PURPOSE: rsl controlling class ** EXPERIMENTAL VERSION **
//
// PROGRAMMING NOTES:
//
// HISTORY: 
...
const char * const thisid = "$Info: rsl_control: EXPERIMENTAL VERSION $";
const char * const rcsid = "$Id: rsl_controlX.cc,v 1.1 1996/03/18 13:27:28 holtrf Exp holtrf $";
...
{	
	...
	ResCall *r_call = new ResCall("ResCall", "");
	r_call->MakeExist(exist_read_only);
	AddReferenceObject(r_call);
...
restable *rtab;
	...
	while(rct)
	{
		...
		cerr << "-- free restable: --\n";
#endif
//		rct->args = NULL;	// object is already freed, but pointer is not NULL -
//		rc_kill(rct);		// so a free(rct->args) inside rc_kill() will seg fault
...
//						<< (r->eval->res_name?r->eval->res_name: "" ) << '\n';
					resource *xyz = make_resource(r->eval);
					if (xyz)
						nr->SetResPtr(xyz);	// would be overridden by name
				}
...
//				cout << " Var " << r->res_name;
		...
		case TYPE_LOOP:
			if (r->eval)
			{
				...
				cout << "Found LOOP.\n";
#endif
				loopResCall *nr = new loopResCall;
				resource *res = make_resource(r->eval);
				nr->SetResPtr(res);	// loop conditional expr to evaluate

				res = make_resource(r->args);
				nr->SetLoopRes(res);	// statement to execute
